export type CatalogItem = {
  slug: string;
  name: string;
  price: number;
  notes: string;
  image: string;
  group?: string;
  joinery?: string;
  assembled?: boolean;
  side?: string;
};

export const drawerBoxes: CatalogItem[] = [
  {
    slug: "dovetail-maple",
    name: "Maple dovetail",
    joinery: "Dovetail",
    assembled: true,
    side: "⅝″",
    price: 42,
    notes: "Clear UV, ¼″ bottom",
    image: "/images/drawers-box.jpg",
  },
  {
    slug: "dovetail-birch",
    name: "Birch dovetail",
    joinery: "Dovetail",
    assembled: true,
    side: "½″",
    price: 34,
    notes: "Volume box. Assembled or KD.",
    image: "/images/gallery-cabinets.jpg",
  },
  {
    slug: "doweled-melamine",
    name: "White melamine",
    joinery: "Doweled",
    assembled: false,
    side: "½″",
    price: 22,
    notes: "Pairs with white Linea. Ships KD.",
    image: "/images/hero-white-kitchen.jpg",
  },
  {
    slug: "doweled-maple",
    name: "Maple doweled",
    joinery: "Doweled",
    assembled: true,
    side: "½″",
    price: 28,
    notes: "Smoother interior, less show.",
    image: "/images/cabinets.jpg",
  },
];

export const hardwareItems: CatalogItem[] = [
  {
    slug: "hinge-overlay",
    name: "Overlay hinge",
    group: "Hinges",
    price: 4.8,
    notes: "Soft-close, ½″ overlay. Pair.",
    image: "/images/gallery-kitchen-work.jpg",
  },
  {
    slug: "hinge-inset",
    name: "Inset hinge",
    group: "Hinges",
    price: 5.4,
    notes: "Soft-close, 3mm tab. Pair.",
    image: "/images/maple.jpg",
  },
  {
    slug: "glide-undermount",
    name: "Undermount glide",
    group: "Glides",
    price: 28,
    notes: "Soft-close, 12″–21″. Pair.",
    image: "/images/drawers-box.jpg",
  },
  {
    slug: "glide-side",
    name: "Side-mount glide",
    group: "Glides",
    price: 16,
    notes: "100 lb class. Pair.",
    image: "/images/gallery-cabinets.jpg",
  },
  {
    slug: "pull-bar",
    name: "Bar pull",
    group: "Decorative",
    price: 6.5,
    notes: "5″ and 8″. Nickel, black, brass.",
    image: "/images/cabinets.jpg",
  },
  {
    slug: "knob-round",
    name: "Round knob",
    group: "Decorative",
    price: 3.2,
    notes: "1¼″. Matching finishes.",
    image: "/images/gallery-wood.jpg",
  },
  {
    slug: "kit-waste",
    name: "Waste / recycle kit",
    group: "Kits",
    price: 86,
    notes: "Fits an 18″ or 21″ Linea base.",
    image: "/images/gallery-kitchen-work.jpg",
  },
  {
    slug: "kit-tipon",
    name: "Tip-on latch",
    group: "Kits",
    price: 9,
    notes: "Handleless openings. Each.",
    image: "/images/cabinets-open.jpg",
  },
];

export const componentItems: CatalogItem[] = [
  {
    slug: "face-frame",
    name: "Face frame",
    group: "Frames",
    price: 48,
    notes: "Maple, poplar, or painted MDF.",
    image: "/images/gallery-classic.jpg",
  },
  {
    slug: "end-panel",
    name: "End panel",
    group: "Panels",
    price: 62,
    notes: "¾″, edged four sides.",
    image: "/images/cabinets-open.jpg",
  },
  {
    slug: "fridge-panel",
    name: "Refrigerator pack",
    group: "Panels",
    price: 180,
    notes: "Two sides + optional upper.",
    image: "/images/gallery-modern.jpg",
  },
  {
    slug: "toekick",
    name: "Toekick",
    group: "Fillers",
    price: 14,
    notes: "4½″ high, cut to run.",
    image: "/images/cabinets.jpg",
  },
  {
    slug: "filler",
    name: "Overlay filler",
    group: "Fillers",
    price: 18,
    notes: "1½″–6″. Same material as the run.",
    image: "/images/hero-white-kitchen.jpg",
  },
  {
    slug: "shelf",
    name: "Adjustable shelf",
    group: "Interior",
    price: 16,
    notes: "¾″, Linea 32mm pattern.",
    image: "/images/gallery-interior.jpg",
  },
  {
    slug: "stretcher",
    name: "Stretcher / nailer",
    group: "Interior",
    price: 9,
    notes: "Sink bases and fridge packs.",
    image: "/images/oak.jpg",
  },
  {
    slug: "finished-back",
    name: "Finished back",
    group: "Panels",
    price: 36,
    notes: "¾″ when the interior is visible.",
    image: "/images/gallery-interior.jpg",
  },
];

export const doorItems: CatalogItem[] = [
  {
    slug: "ridge",
    name: "Ridge",
    group: "Recessed",
    price: 38,
    notes: "Square shaker. ¾″. Maple, alder, MDF.",
    image: "/images/cabinets.jpg",
  },
  {
    slug: "ridge-lite",
    name: "Ridge Lite",
    group: "Glass",
    price: 52,
    notes: "French lite. Mullion optional.",
    image: "/images/gallery-kitchen-work.jpg",
  },
  {
    slug: "vale",
    name: "Vale",
    group: "Slab",
    price: 28,
    notes: "Flat panel. TFL or painted MDF.",
    image: "/images/hero-white-kitchen.jpg",
  },
  {
    slug: "meridian",
    name: "Meridian",
    group: "Miter",
    price: 64,
    notes: "Miter frame, ⅞″. Oak or maple.",
    image: "/images/gallery-classic.jpg",
  },
  {
    slug: "grove",
    name: "Grove",
    group: "Raised",
    price: 58,
    notes: "Raised panel, ¾″. Square or arch.",
    image: "/images/gallery-wood.jpg",
  },
  {
    slug: "grove-lite",
    name: "Grove Lite",
    group: "Glass",
    price: 68,
    notes: "Raised frame, glass panel.",
    image: "/images/maple.jpg",
  },
  {
    slug: "ridge-front",
    name: "Ridge drawer front",
    group: "Fronts",
    price: 22,
    notes: "5-piece to match Ridge doors.",
    image: "/images/gallery-cabinets.jpg",
  },
  {
    slug: "vale-front",
    name: "Vale drawer front",
    group: "Fronts",
    price: 16,
    notes: "Slab to match Vale doors.",
    image: "/images/cabinets-open.jpg",
  },
];

export const accessoryItems: CatalogItem[] = [
  {
    slug: "corbel",
    name: "Corbel",
    group: "Wood",
    price: 42,
    notes: "Maple or oak. Same finish as the doors.",
    image: "/images/gallery-classic.jpg",
  },
  {
    slug: "island-post",
    name: "Island post",
    group: "Wood",
    price: 86,
    notes: "Square or fluted. Cut to height.",
    image: "/images/oak.jpg",
  },
  {
    slug: "bottle-rack",
    name: "Bottle rack",
    group: "Interior",
    price: 54,
    notes: "Lattice or cubby. Wood or TFL.",
    image: "/images/gallery-interior.jpg",
  },
  {
    slug: "valance",
    name: "Valance",
    group: "TFL",
    price: 36,
    notes: "Cut to opening. Matching edge.",
    image: "/images/gallery-kitchen-work.jpg",
  },
  {
    slug: "sheet-stock",
    name: "Extra panel stock",
    group: "Sheet",
    price: 48,
    notes: "Same lot as the run.",
    image: "/images/cabinets-open.jpg",
  },
  {
    slug: "finish-kit",
    name: "Finish completion kit",
    group: "Finish",
    price: 72,
    notes: "Gallon for field parts. Same batch.",
    image: "/images/gallery-wood.jpg",
  },
];

export const specialtyItems: CatalogItem[] = [
  {
    slug: "range-hood",
    name: "Range hood",
    group: "Hoods",
    price: 420,
    notes: "Wall or island. Wood or TFL.",
    image: "/images/maple.jpg",
  },
  {
    slug: "furniture-base",
    name: "Furniture base",
    group: "Bases",
    price: 96,
    notes: "Furniture toe. Island or vanity.",
    image: "/images/gallery-marble.jpg",
  },
  {
    slug: "bun-feet",
    name: "Bun feet",
    group: "Bases",
    price: 28,
    notes: "Turned or block. Set of four.",
    image: "/images/drawers-box.jpg",
  },
  {
    slug: "radius-end",
    name: "Radius end",
    group: "Radius",
    price: 140,
    notes: "Curved end panel or open shelf.",
    image: "/images/gallery-modern.jpg",
  },
  {
    slug: "wine-cubby",
    name: "Wine cubby",
    group: "Wine",
    price: 74,
    notes: "Open lattice. Sized to the opening.",
    image: "/images/gallery-interior.jpg",
  },
];
