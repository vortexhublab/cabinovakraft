...
export function LiveCatalogLines({ detail = "short" }: { detail?: "short" | "summary" }) {
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {lines.map((line) => (
        <ProductCard key={line.slug} product={line} detail={detail} />
      ))}
    </div>
  );
}
    ...
    <>
      ...
      {lineup && (
        <section className="container-site pb-14">
          <h2 className="font-display text-3xl text-ink">Lineup</h2>
